...
static PyMethodDef PYMSEEDMethods[] = {
...
PyMODINIT_FUNC
initpymseed_ext(void)
{
    PyObject *m;
    PyObject *hptmodulus;

    m = Py_InitModule("pymseed_ext", PYMSEEDMethods);
    if (m == NULL) return;
    import_array();

    MseedError = PyErr_NewException("pymseed_ext.error", NULL, NULL);
