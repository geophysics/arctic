...
def calc_inv_A(cfg):
    ...
    try:
        #not possible, if gridpoint section has changed
        raise
...
def read_in_GF(cfg):
    ...
    # otherwise read in Greens functions from NetCDF files 
        ...
        for current_stat in lo_stats:
            ...
            input_filename          = 'gf_v%i_length%i_sampling%i_station_%s.nc' % (gf_version,length_of_orig_gf,int(gf_sampling),current_stat)
                ...
                for idx_k in xrange(3):
                    ...
                    for idx_m in xrange(6):
                        ...
                        if filter_flag == 2 :
                            ...
                            out_data                      =  taper_data(bp_butterworth(taper_data(in_data),gf_sampling,cfg))
                        ...
                        #option 3: ...or not at all
                            ...
                            gf_full[idx_stat,gp,idx_k,idx_m,:] = left_taper_data(in_data)
...
def find_source(cfg):
    ...
    for window_idx in arange(number_of_window_steps):
        ...
        for rec,data in enumerate(data_array_tmp):
            if filter_flag == 2:
                data_array_tmp[rec,:] = taper_data( bp_butterworth(taper_data(data),cfg['data_sampling_rate'] ,cfg ))
            else:
                data_array_tmp[rec,:] = data
...
def gridplot_2pages(cfg,m_cmp):
    ...
    for rec,data in enumerate(data_section):
        if filter_flag == 2:
            data_section[rec,:] = taper_data( bp_butterworth(taper_data(data),cfg['data_sampling_rate'] ,cfg ))
        else:
            data_section[rec,:] = data
    ...
    #set legend
    lgd = 'f%f/%f/%f/10k:km:'%(lon0,maxlat_eff+latannstep/2,lat0)
...
#         page2.draw_layout(outerlayout)
    ...
    for idx_plottrace in arange(len(rec_idxs_plotting_traces)):      
        ...
        real_data              = data_section[receiver_index,:]
...
def rotate_traces(data_x,data_y,angle):
    ...
    in azimuth 80 and y is channel 2, pointing to 170 degree. Then a
...
def present_results(cfg):
    ...
    print '\n...  full Moment Tensor:\npython mopad.py p  %i,%i,%i,%i,%i,%i -D -s 4&' %(int(m_temp[0]/norm_value*100),int(m_temp[4]/norm_value*100),int(m_temp[8]/norm_value*100),int(m_temp[1]/norm_value*100),int(m_temp[2]/norm_value*100),int(m_temp[5]/norm_value*100))
    print 'python mopad.py c  %f,%f,%f,%f,%f,%f -t sdr -y &' %(m_temp[0]/norm_value,m_temp[4]/norm_value,m_temp[8]/norm_value,m_temp[1]/norm_value,m_temp[2]/norm_value,m_temp[5]/norm_value)
    print 'python mopad.py d  %f,%f,%f,%f,%f,%f -p full,iso,devi,iso_perc,clvd_perc -y &' %(m_temp[0]/norm_value,m_temp[4]/norm_value,m_temp[8]/norm_value,m_temp[1]/norm_value,m_temp[2]/norm_value,m_temp[5]/norm_value)
...
def set_general_parameters(cfg):
    ...
    print 'Analysing time interval from ', starttime_of_main_window,' to ',endtime_of_main_window,' in ',  number_of_window_steps,' step(s) of %.3f seconds \n '%(stepsize_of_moving_window)
