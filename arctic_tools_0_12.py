...
def read_in_GF(cfg):
    ...
    #set filename for saving output            
    if filter_flag == 1 or filter_flag == 2:
        GF_pickle_filename          = 'bandpass_filtered_current_GF_pickle_file_for_stations_'+'_'.join(lo_stats)+'.pp'
    else:
    ...
    # otherwise read in Greens functions from NetCDF files 
        ...
        print 'approx size of GF tensor to keep in memory: %i MB \n'%(int(4*n_stations* n_gridpoints*18*n_time/1024**2 ))
        #setup array for holding Greens functions
        gf              = zeros((n_stations, n_gridpoints,  3,6, n_time),float32)

        if _debug:
            print 'GF dimensions:', shape(gf),'\n'
            print 'approx size of file: %i MB '%(int(4*n_stations* n_gridpoints*18*n_time/1024**2 ))
        ...
        for current_stat in lo_stats:
            ...
            input_filename          = 'gf_v%i_length%i_sampling%i_station_%s.nc' % (gf_version,length_of_orig_gf,gf_sampling,current_stat)
            ...
            for gp in xrange(n_gridpoints):
                ...
                #for every gridpoint
                idx_s_real = int(current_lo_gridpoints[gp])-1
                for idx_k in xrange(3):
                    ...
                    for idx_m in xrange(6):
                        ...
                        in_data        = tmp_gf2[current_column,:n_time ]

                        #option 2: filter data with butterworth...
                        if filter_flag == 2 :

                            out_data                      =  bp_butterworth(in_data,gf_sampling,cfg)
                            gf[idx_stat,gp,idx_k,idx_m,:] = out_data
                            
                        #option 1:...or boxcar function...
                        elif filter_flag == 1 :

                            out_data = bp_boxcar(taper_data(in_data),gf_sampling, cfg)
                            gf[idx_stat,gp,idx_k,idx_m,:] = out_data

                        #option 3: ...or not at all
                        else:                            
                            gf[idx_stat,gp,idx_k,idx_m,:] = in_data
        ...
        print 'dimensions of GF array: ', shape(gf)
...
def find_source(cfg):
    ...
    for window_idx in arange(number_of_window_steps):
        ...
        if _debug:
            print window_idx, shape(b)
            #exit()
        
        #estimation of moment-tensor:
        if not estimate_m(b[window_idx,:,:], cfg):
...
def estimate_res( data, time_axis, M_array, cfg):
    ...
    if key == 0:
        ...
        tmp_integral_collection  = zeros((n_receivers),float)
        ...
        for gp in xrange(n_gridpoints):
            ...
            for current_receiver in lo_receivers:
                ...
                current_data = data[receiver_index,:][:]
...
#                         print 'data for station, channel... ',current_station,current_channel, ' in window ', 30 + 10 * (station_index+1) +channel_index
                ...
                tmp_integral_collection[receiver_index] = tmp_integral
               #  if current_station == 'HM04' and gp == 121:
#                     print  gp,current_station,'\t', '\t',sum(current_data),'\t',sum(tmp_synth),'   \t', sum(tmp_m),'\t',sum(tmp_gf) ,'\n'
...
#                     exit()
            ...
            res[gp] = sum(tmp_integral_collection[:])
           #  if 110<gp<130:
...
def taper_data(data):
    """ Taper data with exponential function. First and last 10\% of the data trace are support of the taper function. Data must have a minimum of 50 data samples; otherwise the original data is returned.
    ...
    if (N <= 50 ):
        ...
        idx90                = N-idx10
        int_x_axis           = arange(N)
        aa                   = 1./(0.1*N)*math.log(N)*math.log10(N)
        bb                   = float(N)
        #arbitrarily chosen function 
        taper[1:idx10]        = 1./(1. + bb*exp(-aa/2.*int_x_axis[1:idx10]))

        taper[N-2:idx90-1:-1]= taper[1:idx10]
        taper[idx10:idx90]    = 1
        tapered_data         = zeros(len(taper))

        if shape(data) != shape(taper):
            print 'ERRRRRROOOORRRR -- datalength not equal taper length !!    '
...
def gridplot_2pages(cfg):
    ...
    # one row per station plus 1 header row
    n_traceplots_per_column = len(lo_CS)+1# int(round(n_traces_in_plot/2.))
    ...
    if (eastbound-westbound) > (northbound-southbound):
        widget.set_vertical( 10*cm *asp_rat )
    else:
        widget.set_vertical( 12*cm )
        widget.set_horizontal( 10*cm /asp_rat  )
    ...
    #rectangular plot expected - no calculation of aspects needed, neither prj, rng
    widget.set_horizontal( 10*cm )
    widget.set_vertical( 10*cm )
    #append the completely configured widget to list of all widgets
    allwidgets_p1.append( widget )
    ...
    #calculation of effective beachball size and plotting for each gridpoint 
    size_of_optimum_in_plot = 1.#exp(-(  ((100. - max(gridplotdata_z[:,2]))/100.)**2/0.001))
    #loop over all gridpoints in chosen horizontal layer - 
    for plotline in arange(len(gridplotdata_z[:,0])):
        scale  = size_of_optimum_in_plot#exp(-(  ((100. - gridplotdata_z[plotline,2])/100.)**2/0.001))/size_of_optimum_in_plot
        #set effective size for current beachball
    ...
    #set coordinate array for respective receivers
    plot_rec_sym_coo = zeros((len(lo_CR),3))
    jjj = 0
    for iii in rec_idxs_plotting_traces:
        plot_rec_sym_coo[jjj,2] = 1
        plot_rec_sym_coo[jjj,0] = lonrec[iii]
    ...
    #set legend
    lgd = 'f%f/%f/%f/10k:km:'%((maxlon_eff+minlon_eff)/2.,maxlat_eff+latannstep/2,(maxlat_eff+minlat_eff)/2.)
    ...
    for plotline in arange(len(gridplotdata_z[:,0])):
        #set effective size for current beachball
        scale  = 1.5#*exp(-(  ((100. - gridplotdata_z[plotline,2])/100.)**2/0.001))/size_of_optimum_in_plot
        psm    = 'm%g'%(bbs*scale)
        #plot psmeca element into the active widget ('plotwidget') 
        page1.psmeca(in_rows=[gridplotdata_z[plotline,:].tolist()], M=ssm, S=psm, W=pcl, Z=ctb, R=True, J=True , *plotwidget.XY() )
    ...
    #plot colour scale right of grid
    pos = '%f/%f/%f/%f'%(6,3,3,0.3)#(maxlon_eff+1.5*lonannstep/2,(maxlat_eff+minlat_eff)/2.,3*cm,0.5*cm)
    ann = '%i:VR:/:%s:'%(25,'%')
    skp = ''
    fil = '%s'%( path.realpath(path.abspath(path.join(cfg['base_dir'],cfg['colourtable']))))
    ...
    #set layout type
    #every station gets one row
    #in every row: stationnam, N,E,Z, station weight
    outerlayout   = GridLayout(5,n_traceplots_per_column)
    
    #find width w and height in points
    ...
    #set sub widget for every trace
    for icol in arange(5):
        for irow in arange(n_traceplots_per_column):

            #set layout type
            if icol in [1,2,3] and irow > 0:
                ...
                inner_layout_trace =  CenterLayout()
                outerlayout.set_widget(icol,irow,inner_layout_trace)
                widget              = inner_layout_trace.get_widget()
    ...
    sorted_lo_CS      = sort(lo_CS)
    sort_order_lo_CS  = argsort(lo_CS)
    dict_sort_lo_CS_alphabet = dict()
    for idx,sta in enumerate(lo_CS):
        dict_sort_lo_CS_alphabet[sta]                   = sort_order_lo_CS[idx]
        dict_sort_lo_CS_alphabet[sort_order_lo_CS[idx]] = sta
    dict_sort_lo_CC_NED = {'N':0,'E':1,'Z':2,'0':'N','1':'E','2':'Z'}

    for station in lo_CS:
        row         = dict_sort_lo_CS_alphabet[station]+1
        plotwidget  = allwidgets_p2[row]
        ann        = ':%s:' % (station)
        rng        = (0,1,0,1 )
        #page2.psbasemap( J='x1/1', R=rng,B=ann,*plotwidget.XY()  ) 
        
        #print station name here !!!!!
        #
        #

        plotwidget     = allwidgets_p2[ 4*len(lo_CS) + row ]
        station_weight = cfg['station_weights_dict'][station]
        #print weight here
        #
        #
    ...
    for idx_plottrace in arange(len(rec_idxs_plotting_traces)):      
        ...
        current_station        = current_receiver.split('.')[0]
        current_channel        = current_receiver.split('.')[1]
        row                    = dict_sort_lo_CS_alphabet[current_station]+1 
        column                 = dict_sort_lo_CC_NED[current_channel.upper()[-1]]+1
        
        #set current widget index - including two already used widgets for upper part of plot 
        idx_plotwidget         = len(lo_CS)*column + row
        ...
        #set annotations
        ann        = '%g/%g::SW' % (int(float(max(time_axis))/5+1),0.5)#, current_receiver)

        if column ==1 :
            ann        = '%g/%g:%s:SW' % (int(float(max(time_axis))/5+1),0.5, current_station)

            
        current_gmtdefaults = {}
        current_gmtdefaults['ANNOT_FONT_SIZE_SECONDARY'] = '7p'
        current_gmtdefaults['ANNOT_FONT_SIZE_PRIMARY'] = '3p'
...
def present_results(cfg):
    ...
    norm_value = max( [abs(m_opt_mat.min() ),abs(m_opt_mat.max() )])
    M0 = sqrt(abs(decomp_out[8]*norm_value))
    MW = 2./3.*( log10(M0*10**7)  -16.0)
    ...
    print '\n...  full Moment Tensor:\n  %f,%f,%f,%f,%f,%f ' %(m_temp[0]/norm_value,m_temp[4]/norm_value,m_temp[8]/norm_value,m_temp[1]/norm_value,m_temp[2]/norm_value,m_temp[5]/norm_value)
...
def build_xml_hardcode(cfg):
    ...
    vr_opt                 = opt_source_dict['VR_opt']
    source_opt             = opt_source_dict['source']
    source_opt_coords      = source_coordinates[source_opt]
    ...
    set_standard_tag(or_M, 'scalarMoment',  decomp_out[8])
    set_standard_tag(or_M, 'scalarMoment_unit','10e17 Nm')
    ...
    for current_stat in cfg['list_of_stations_with_data']:
        fn_tmp   =  'gf_v%i_length%i_sampling%i_station_%s.nc' % (gf_version,length,gf_sampling,current_stat)
...
def set_general_parameters(cfg):
    ...
    # eventtime
    eventtime =  round(read_datetime_to_epoch(cfg['datetime']))
    ...
    print 'Analysing time interval from ', starttime_of_main_window,' to ',endtime_of_main_window,' in ',  number_of_window_steps,' step(s) \n '
...
def read_station_coordinates(cfg):
    ...
    stat_coords = zeros((s_number,5),float)
    distmax     = max(lo_dist)
...
def set_raw_data_for_main_window(cfg):
    ...
    mseed_data_pile        = pymseed.MSeedPile(cfg['list_of_datafiles'])
    ...
    for ii in mseed_data_pile.chopper(tmin=starttime,tmax=endtime+deltat,want_incomplete=False):
...
def find_weights_for_stations(cfg,station_distance_dict,lo_dist):
    ...
    if not (weighting_flag == 2 or weighting_flag == 1 or weighting_flag == 0 ):
        weighting_flag = 0
    ...
    if weighting_flag == 1:
        ...
        print 'stations weighted by distance'
...
#                         print  temp_red_mat
        ...
        #if gp == 0:
        #    print '\n\nA[0] - Summe - neu  ', sum(reduced_A),'\n'
        #    exit()
            #print '\n',stationname,station_weight,station_weight_new, '\n'
            #if gp == 0 or gp == n_gridpoints-1:
            #    print temp_A[:,:],'\n'
...
#     print '\n'
    ...
    FH_inv_A_out.close()
    print 'inverse correlation matrix inv_A for reduced number of data containing traces written to file'            
...
def rescale_M_opt(cfg):
    ...
    For all data traces, used in the inversion, the respective synthetic is calculated by using the moment tensor solution M_opt. The latter is being re-scaled to minimise the total misfit of all data-synthetics comparisons. Re-scaling is been done by a least squares algorithm.
    ...
    data_synth_array = zeros(( n_traces, 2, len(data[0,:]) ))    
    ...
    if not  estimate_res(data, time_axis, M_scaled, cfg):
